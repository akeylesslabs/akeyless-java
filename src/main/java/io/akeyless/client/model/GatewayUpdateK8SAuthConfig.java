...
public class GatewayUpdateK8SAuthConfig {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("access-id", "cluster-api-type", "disable-issuer-validation", "json", "k8s-auth-type", "k8s-ca-cert", "k8s-client-certificate", "k8s-client-key", "k8s-host", "k8s-issuer", "name", "new-name", "rancher-api-key", "rancher-cluster-id", "signing-key", "token", "token-exp", "token-reviewer-jwt", "uid-token", "use-gw-service-account"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("access-id", "k8s-host", "name", "new-name", "signing-key"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GatewayUpdateK8SAuthConfig.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in GatewayUpdateK8SAuthConfig is not found in the empty JSON string", GatewayUpdateK8SAuthConfig.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GatewayUpdateK8SAuthConfig.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `GatewayUpdateK8SAuthConfig` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("cluster-api-type") != null && !jsonObj.get("cluster-api-type").isJsonNull()) && !jsonObj.get("cluster-api-type").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `cluster-api-type` to be a primitive type in the JSON string but got `%s`", jsonObj.get("cluster-api-type").toString()));
      }
