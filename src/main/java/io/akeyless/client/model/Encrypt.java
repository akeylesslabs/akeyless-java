...
public class Encrypt {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("display-id", "encryption-context", "input-format", "item-id", "json", "key-name", "plaintext", "token", "uid-token", "version"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Encrypt.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in Encrypt is not found in the empty JSON string", Encrypt.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Encrypt.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `Encrypt` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
