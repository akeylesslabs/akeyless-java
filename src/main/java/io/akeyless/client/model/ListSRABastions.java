...
public class ListSRABastions {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("allowed-urls-only", "json", "token", "uid-token"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ListSRABastions.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in ListSRABastions is not found in the empty JSON string", ListSRABastions.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ListSRABastions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ListSRABastions` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
