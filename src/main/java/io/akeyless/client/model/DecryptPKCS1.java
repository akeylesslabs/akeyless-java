...
public class DecryptPKCS1 {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("ciphertext", "display-id", "item-id", "json", "key-name", "output-format", "token", "uid-token", "version"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DecryptPKCS1.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in DecryptPKCS1 is not found in the empty JSON string", DecryptPKCS1.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DecryptPKCS1.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `DecryptPKCS1` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
