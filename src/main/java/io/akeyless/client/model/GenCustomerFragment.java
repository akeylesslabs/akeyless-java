...
public class GenCustomerFragment {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("description", "hsm-key-label", "json", "metadata", "name", "type"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GenCustomerFragment.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in GenCustomerFragment is not found in the empty JSON string", GenCustomerFragment.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GenCustomerFragment.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `GenCustomerFragment` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("hsm-key-label") != null && !jsonObj.get("hsm-key-label").isJsonNull()) && !jsonObj.get("hsm-key-label").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `hsm-key-label` to be a primitive type in the JSON string but got `%s`", jsonObj.get("hsm-key-label").toString()));
      }
