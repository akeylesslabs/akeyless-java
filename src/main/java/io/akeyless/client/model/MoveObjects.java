...
public class MoveObjects {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("json", "objects-type", "source", "target", "token", "uid-token"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MoveObjects.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in MoveObjects is not found in the empty JSON string", MoveObjects.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MoveObjects.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `MoveObjects` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("objects-type") != null && !jsonObj.get("objects-type").isJsonNull()) && !jsonObj.get("objects-type").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `objects-type` to be a primitive type in the JSON string but got `%s`", jsonObj.get("objects-type").toString()));
      }
