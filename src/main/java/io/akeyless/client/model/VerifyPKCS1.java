...
public class VerifyPKCS1 {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("display-id", "hash-function", "input-format", "item-id", "json", "key-name", "message", "prehashed", "signature", "token", "uid-token", "version"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("key-name", "message", "signature"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VerifyPKCS1.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in VerifyPKCS1 is not found in the empty JSON string", VerifyPKCS1.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VerifyPKCS1.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `VerifyPKCS1` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
