...
public class CreateLdapTarget {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("bind-dn", "bind-dn-password", "comment", "description", "json", "key", "ldap-ca-cert", "ldap-url", "max-versions", "name", "server-type", "token", "token-expiration", "uid-token"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CreateLdapTarget.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in CreateLdapTarget is not found in the empty JSON string", CreateLdapTarget.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CreateLdapTarget.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `CreateLdapTarget` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
