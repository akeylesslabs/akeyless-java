...
public class VerifyJWTWithClassicKey {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("display-id", "json", "jwt", "required-claims", "token", "uid-token", "version"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("display-id", "jwt", "required-claims", "version"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VerifyJWTWithClassicKey.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in VerifyJWTWithClassicKey is not found in the empty JSON string", VerifyJWTWithClassicKey.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VerifyJWTWithClassicKey.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `VerifyJWTWithClassicKey` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("required-claims").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `required-claims` to be a primitive type in the JSON string but got `%s`", jsonObj.get("required-claims").toString()));
      }
