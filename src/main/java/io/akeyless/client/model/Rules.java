...
public class Rules {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("admin", "path_rules"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Rules.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in Rules is not found in the empty JSON string", Rules.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Rules.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `Rules` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("path_rules") != null && !jsonObj.get("path_rules").isJsonNull()) {
        ...
        if (jsonArraypathRules != null) {
          ...
          if (!jsonObj.get("path_rules").isJsonArray()) {
            throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `path_rules` to be an array in the JSON string but got `%s`", jsonObj.get("path_rules").toString()));
          }
