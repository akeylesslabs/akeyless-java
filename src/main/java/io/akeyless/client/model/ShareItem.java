...
public class ShareItem {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("accessibility", "action", "emails", "item-name", "json", "share-type", "shared-token-id", "token", "ttl", "uid-token", "view-once"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("action", "item-name"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ShareItem.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in ShareItem is not found in the empty JSON string", ShareItem.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ShareItem.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ShareItem` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("share-type") != null && !jsonObj.get("share-type").isJsonNull()) && !jsonObj.get("share-type").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `share-type` to be a primitive type in the JSON string but got `%s`", jsonObj.get("share-type").toString()));
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("shared-token-id") != null && !jsonObj.get("shared-token-id").isJsonNull() && !jsonObj.get("shared-token-id").isJsonArray()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `shared-token-id` to be an array in the JSON string but got `%s`", jsonObj.get("shared-token-id").toString()));
      }
