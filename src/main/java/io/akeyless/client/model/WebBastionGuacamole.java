...
public class WebBastionGuacamole {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("keyboard_layout"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!WebBastionGuacamole.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in WebBastionGuacamole is not found in the empty JSON string", WebBastionGuacamole.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!WebBastionGuacamole.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `WebBastionGuacamole` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("keyboard_layout") != null && !jsonObj.get("keyboard_layout").isJsonNull()) && !jsonObj.get("keyboard_layout").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `keyboard_layout` to be a primitive type in the JSON string but got `%s`", jsonObj.get("keyboard_layout").toString()));
      }
  }
